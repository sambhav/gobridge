...
// compileStreamBinding reuses unary parameter/schema validation while replacing
// invocation with a producer callback. No producer runs during generation.
func compileStreamBinding(name string, fn reflect.Value, receiver func() reflect.Value, names []string) (operation, error) {
	t := fn.Type()
	last := t.In(t.NumIn() - 1)
	offset := 0
	if receiver != nil {
	...
	if t.NumIn() <= offset || t.In(offset) != contextType {
		return operation{}, fmt.Errorf("stream producer requires context.Context")
	}
	inputs := make([]reflect.Type, t.NumIn()-1)
	for i := range inputs {
		inputs[i] = t.In(i)
	}
	shape := reflect.FuncOf(inputs, []reflect.Type{last.In(0)}, false)
	dummy := reflect.MakeFunc(shape, func([]reflect.Value) []reflect.Value { panic("schema-only function") })
	op, err := compileBinding(name, dummy, receiver, names)
	if err != nil {
		return operation{}, err
	}
	op.call = nil
	op.stream = func(ctx context.Context, raw json.RawMessage, yield func(any) error) error {
		input, err := decodeInput(raw, op.in)
		if err != nil {
			return Failure("invalid_argument", err.Error())
		}
		...
		args = append(args, reflect.MakeFunc(last, func(values []reflect.Value) []reflect.Value {
			err := yield(values[0].Interface())
			result := reflect.Zero(errorType)
			if err != nil {
				result = reflect.ValueOf(&err).Elem()
			}
