...
var typescriptMethodReserved = func() map[string]bool {
	m := map[string]bool{}
	for _, name := range strings.Fields("constructor prototype then calls batch stream call start close schema control toString toLocaleString valueOf hasOwnProperty isPrototypeOf propertyIsEnumerable") {
		m[name] = true
	}
	return m
}()
...
func (r *Registry) GenerateTypeScript(w io.Writer, class, binary string, options ...Option) error {
	...
	reserved := map[string]bool{"calls": true, "schema": true, "configure": true, "session": true, "shutdown": true, "Promise": true, "Record": true, "Uint8Array": true}
	if reserved[class] {
	...
	fmt.Fprintln(&b, "import { Call as _bridgeCall, Client as _bridgeClient, DefaultControl as _bridgeDefaultControl, resolveBinary as _bridgeResolveBinary, parseSchema as _bridgeParseSchema, encode as _bridgeEncode, compileCodec as _bridgeCompileCodec } from \"gobridge-runtime\";")
	...
	for _, name := range names {
		if len(types[name].Enum) > 0 {
			fmt.Fprintf(&b, "export const %s = {\n", name)
			for _, v := range types[name].Enum {
				literal := string(v.Value)
				if types[name].Kind == "int64" || types[name].Kind == "uint64" {
					literal += "n"
				}
				fmt.Fprintf(&b, "  %s: %s,\n", tsQuote(v.Name), literal)
			}
	...
	for index, op := range schema.Operations {
		fmt.Fprintf(&b, "const _bridgeInput%d = _bridgeCompileCodec(schema.operations[%d]!.input);\n", index, index)
		fmt.Fprintf(&b, "const _bridgeOutput%d = _bridgeCompileCodec<%s>(schema.operations[%d]!.output);\n", index, tsType(op.Output), index)
	}
	fmt.Fprintln(&b)
	...
	for index, op := range schema.Operations {
		...
		if len(op.Input.Fields) > 0 {
			params = "params: " + op.Input.Name
			input = "params"
		}
		fmt.Fprintf(&b, "  %s(%s): _bridgeCall<%s> { return new _bridgeCall(%s, _bridgeInput%d.encode(%s), value => _bridgeOutput%d.decode(value)); },\n", tsOperationName(op), params, tsType(op.Output), tsQuote(op.Name), index, input, index)
	}
	fmt.Fprint(&b, "};\n\n")
	fmt.Fprintf(&b, "export class %s extends _bridgeClient {\n", class)
	fmt.Fprint(&b, "  readonly calls = calls;\n")
	defaultOptions := " = {}"
